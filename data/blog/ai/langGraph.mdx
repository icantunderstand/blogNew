...
LangGraph 利用 StateGraph 将代理流程建模为有状态的图，开发者可以以工作流的方式精细编排节点、状态与条件边，构建高度定制化的代理交互.

## 核心概念

- **状态（State）**：存储代理在执行过程中需要维护的数据结构，可以包含任意键值对，节点函数可以读写状态
- **节点（Node）**：执行特定功能的函数，接受当前 state 作为入参，在函数逻辑中基于当前 state 生成并返回新的 state
- **边（Edge）**：定义节点之间的连接关系，决定在当前的 state 下哪个节点应该执行
- **StateGraph**：整个代理流程的核心数据结构，包含所有节点、边和状态定义，负责管理执行流程和状态更新

### 状态(State)

<CollapsibleCode title="状态示例" language="python">
```python
...
节点是执行实际工作的函数，接收状态并返回状态更新,节点可以调用函数、工具或 LLM 来完成任务.
...
<CollapsibleCode title="条件边/StateGraph示例" language="python">
```python
