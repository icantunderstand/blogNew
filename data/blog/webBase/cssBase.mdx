...
Dirty（脏标记） 是浏览器用来标识 DOM 元素或渲染对象状态已改变、需要重新计算的标志位，通过标记需要重新计算的元素来避免不必要的全局重排。

1. JavaScript 修改 DOM/CSS
2. 标记相关节点为 Dirty
3. 浏览器调度布局任务
4. 遍历 Dirty 节点
5. 重新计算布局（Reflow）
...
## transform/translate/trasition

* transform 提供2D/3D变换函数，可以对元素进行形变
* trasition 过渡动画控制器, 定义某个属性从A到B的过渡过程
* translate 专职移动
...
* rem 是相对单位，它的基准是根元素html的 font-size
* 1rem 永远等于根元素 font-size 的值
