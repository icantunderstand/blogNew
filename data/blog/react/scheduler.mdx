...
```
scheduleCallback(priority, cb, { delay? })
         │
         ├── delay > 0 ──→ timerQueue（等待 → 到期后移入 taskQueue）
         │
         └── delay = 0 ──→ taskQueue（通过 MessageChannel 异步执行）
                                │
                          每帧 5ms 时间切片
                          超时任务会被"插队"优先执行
```

### react与Scheduler包结合实现任务中断恢复

#### 任务恢复
通过workInProgress记录当前执行的fiber节点，当任务被中断恢复的时候从中断的地方继续执行

#### 任务中断
用过shouldYield判断是否需要中断，当返回true的时候说明需要中断任务的执行
