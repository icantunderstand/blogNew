...
**async/await 是 Generator 的语法糖 + Promise 作为异步载体** 
...
function pipe(...funcs) {
  ...
  return (...args) =>
    funcs.reduce((acc, fn, idx) => (idx === 0 ? fn(...acc) : fn(acc)), args)
...
function parseObj(obj) {
  ...
  for (const [key, value] of sortedEntries) {
    // 过滤空字符串（处理 '.a' 或 'a..b' 这种情况）
    const arr = key.split('.').filter(k => k !== '')
    
    // 空键处理
    if (arr.length === 0) {
      console.warn(`无效的键: "${key}"，跳过`)
      skipped.push(key)
      continue
    }
    ...
    for (let i = 0; i < arr.length; i++) {
      if (i < arr.length - 1) {
        ...
        // 检查路径冲突（包括 null 检查） 有值且不是对象 就无法扩展
        if (current !== undefined && (current === null || typeof current !== 'object')) {
          console.warn(`路径冲突: "${key}" 与已存在的值 ${JSON.stringify(current)} 冲突，跳过此路径`)
          skipped.push(key)
          isConflict = true
          break
        }
        ...
        // 检查是否会覆盖已存在的对象  有值且是一个对象
        if (current !== undefined && typeof current === 'object' && current !== null) {
          console.warn(`路径冲突: "${key}" 会覆盖已存在的对象 ${JSON.stringify(current)}`)
        }
        
        // 深拷贝对象/数组，避免引用问题
        if (typeof value === 'object' && value !== null) {
          try {
            prev[arr[i]] = JSON.parse(JSON.stringify(value))
          } catch (e) {
            // JSON 无法序列化的对象（如函数、循环引用等）
            console.warn(`无法深拷贝值: "${key}"，使用浅拷贝`)
            prev[arr[i]] = Array.isArray(value) ? [...value] : { ...value }
          }
        } else {
          prev[arr[i]] = value
        }
  ...
  // 可选：返回跳过的键信息
  if (skipped.length > 0) {
    console.log(`共跳过 ${skipped.length} 个键:`, skipped)
  }
...
/**
 * 递归统计一个 Treasure（或 Email）节点下所有的 gold 和 skin 总量
 * @param {object} node - 含有 gold, skin, treasure[] 的节点
 * @returns {{ totalGold: number, totalSkin: number }}
 */
function collectStats(node) {
  let totalGold = node.gold || 0;
  let totalSkin = node.skin || 0;

  if (node.treasure && node.treasure.length > 0) {
    for (const child of node.treasure) {
      const childStats = collectStats(child);
      totalGold += childStats.totalGold;
      totalSkin += childStats.totalSkin;
    }
  }

  return { totalGold, totalSkin };
}

/**
 * 处理邮件列表，将符合条件的邮件宝藏加入背包
 * @param {Email[]} emailList
 */
function ReadEmailList(emailList) {
  for (const email of emailList) {
    // 跳过已读邮件
    if (email.isRead) continue;

    // 递归统计这封邮件的全部 gold 和 skin
    const { totalGold, totalSkin } = collectStats(email);

    // 判断皮肤是否超出背包上限
    if (playerBag.skin + totalSkin > MAX_SKIN) {
      // 皮肤放不下，跳过，不标记已读
      continue;
    }

    // 放得下：加入背包，标记已读
    playerBag.gold += totalGold;
    playerBag.skin += totalSkin;
    email.isRead = true;
  }
}
...
function calculate(s) {
  const stack = [];
  let result = 0;   // 当前层的累计结果
  let num = 0;      // 当前正在读取的数字
  let sign = 1;     // 当前符号，+1 或 -1
  ...
  for (const ch of s) {
    if (ch >= '0' && ch <= '9') {
      ...
      result += sign * num;
      num = 0;
      // 弹栈还原外层：括号结果 * 外层符号 + 外层累计
      result *= stack.pop(); // 外层 sign
      result += stack.pop(); // 外层 result
    }
  }
...
* interface：专门用于描述对象的结构，是面向对象风格的类型声明。

* type：更通用的类型别名，可以描述任何类型，包括联合类型、元组、基础类型等

#### 相同&差异
* interface支持声明合并  type 支持
* 都支持扩展
